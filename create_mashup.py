...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--handle', default = login_handle, help = 'Codeforces handle')
    ...
    parser.add_argument('--min_contest_id', default = '500', type = int, help = 'Minimum contest id (useful for filtering out old problems)')
    ...
    config = vars(args)
    print(config)
    (handle, mashup_name, duration, min_contest_id, min_rating, max_rating, problem_count) = (args.handle, args.name, args.duration, args.min_contest_id, args.min_rating, args.max_rating, args.problem_count)
    problems = select_problems(handle, min_rating, max_rating, problem_count, min_contest_id)
